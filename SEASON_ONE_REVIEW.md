...
| 11  | **There is no backup.** `GET /api/mlb/storage-status` on production returns `upstash_configured: false`, so `saveToUpstash` returns immediately and always has. The whole subsystem is inert: `slimForBackup`, `serializeForUpstash`, the dated snapshots, `GET /api/admin/db-backups` and `POST /api/admin/db-restore` (both 503 `backups_unavailable`), the boot-time is-the-backup-newer check, and `.github/workflows/refresh-db-fixture.yml` — which is why `tests/fixtures/db.sample.json` has never existed. Neither admin endpoint is called from `app.js`, so nothing ever surfaced it. The size problem is real but _latent_: enabling Upstash today would fail anyway, the slimmed payload being ~4.83 MB against a ~1 MB cap. | server.js:391, 3479, 3500    |
...
`slimForBackup` strips the daily rows because they are big. Size is the wrong criterion —
replaceability is. Almost everything in `db.json` can be rebuilt: the stat rows from the MLB Stats
API (`POST /api/mlb/backfill`), the pools and team maps from the player catalog
(`bootstrapPlayerPools`). What cannot be rebuilt from anywhere is `roster_dates`, `swaps`,
`initial_submissions`, `period_submissions`, `rosters` and `roasts` — the scoring invariant's own
sources and the league's written history. On production that is **0.25 MB for 2026**. A backup of
only that fits Upstash's free tier many times over, and it is the only part whose loss is
unrecoverable. Losing every stat row costs a re-sync; losing the swap log costs the season.

Whatever carries it, the Render disk should stop being the only copy. It is durable against
restarts and redeploys, which is what `durable: true` reports — but not against the failure class
this app actually has, which is a bad write. A stale full-season save wiped `sd.rosters` on
2026-06-08, and a boot against a test fixture rewrote `managers_seed.json` on 2026-09-02. Neither
is something a disk protects you from, and there is no point-in-time recovery: `score_snapshots` is
a 21-day diagnostic trail of per-manager totals, not a restore path.
