# WMMC — Decisions Log (archive)

Entries older than 2026-07-29, split out of `MEMORY.md` so session startup does not load the
whole history. Nothing here is retired or superseded by virtue of being archived — it is the
same decisions log, and the index in `MEMORY.md` links straight into it.
