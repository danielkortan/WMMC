...
- **`tests/serverMirrors.test.js` mechanizes the "edit both copies" rule** for the pairs it covers — it reads `server.js` as text and fails if a mirrored block has drifted from its `js/` original. It runs no server code. When you intentionally change a mirrored helper, change it in `js/` too and the test goes green again. It currently guards `normalizeName`, `shortManagerNames`, `js/history.js` and `js/playoffCommentary.js`; extending it to the older pairs (`SCORING`, `detectScoreSwings`, the odds engine) would be a straight win.
