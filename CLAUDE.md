...
- **THE CORE SCORING INVARIANT (this is the heart of the app — protect it above all else).** A
  ...
  - **Any change touching managers, player lists, roster windows, swaps, or scoring must be vetted
    ...
    payload.
- `SCORING` and `SEASON_SCHEDULE` are hardcoded in **both** `server.js` and `js/scoring.js` and must be kept in sync — every edit goes in both files. (`tests/serverMirrors.test.js` now enforces this by value.) (They were moved out of `app.js` during modularization; `app.js` now reads them from `window`, populated by `js/index.js`.)
...
- **`tests/serverMirrors.test.js` mechanizes the "edit both copies" rule** for the pairs it covers — it reads `server.js` as text and fails if a mirrored block has drifted from its `js/` original. It runs no server code. When you intentionally change a mirrored helper, change it in `js/` too and the test goes green again. It currently guards `normalizeName`, `shortManagerNames`, `shortenManagerNamesInSlack`, `js/history.js`, `js/playoffCommentary.js`, `js/roundPreview.js`, `js/seasonRecap.js`, the elimination ladder (`lastRoundPlayed`/`isManagerActiveInRound`/`isManagerInRound` from `js/eligibility.js`), `js/backupSet.js`, `js/statRetention.js`, `js/rosterWindows.js`, `js/seasonArchive.js`, `periodStartForRound`, `detectScoreSwings`, `checkSwapLimit`, `SCORING`, `SEASON_SCHEDULE` and ten functions of the odds engine — **every mirrored pair in the repo is now guarded.** `SCORING` and `SEASON_SCHEDULE` are compared by VALUE rather than as text, because prettier keeps the server's pitching table on one line and the `js/` entries carry a `label` the server's do not; `SCORING` is parsed per section, since `BB` exists in both tables and a flat map would collapse the one drift most worth catching. It has already earned its keep — adding the odds-engine pairs immediately caught a comment that had gone missing from the `server.js` copy of `makeNormalSampler`.
...
- **The sync stores a row for every player in every game, and 85% of them are for players nobody
  rostered.** `js/statRetention.js` (canonical, unit-tested) ↔ `server.js` is the write-side filter
  that stops it, on the same must-stay-identical footing as the pairs above. **It is OPT-IN per
  season** (`sd.stat_retention`, default `'all'` = today's behaviour to the byte), because 2026's
  rows are already written and this is for 2027 onward. The keep-set is deliberately PERMISSIVE —
  under-keeping loses points and over-keeping costs bytes, and those are not the same mistake — so
  it unions the roster arrays, `roster_dates`, BOTH sides of every swap **including pending ones**,
  every submission including unapproved ones, and the `sd.held_players` escape hatch. Pending swaps
  are not an oversight: a swap approved Thursday can carry a Tuesday `add_date`, and Tuesday's rows
  must already exist. It also stands itself down below `RETENTION_MIN_KEEP` (40) so a half-loaded
  season stores everything rather than nothing. The one hazard it cannot close — a player NOBODY
  had, given a back-dated add — is repaired by re-syncing that week (`POST /api/mlb/sync`), which
  is why this is a filter over regenerable data and never a delete. The manual stat-entry endpoint
  and `serverAutoAdvancePlayers` are deliberately NOT filtered: the first is a commissioner naming a
  player outright, the second only ever writes rows for players who are by definition in the set.

- **There are now THREE backups and they answer different questions — do not collapse them.**
  Render's disk snapshot (every 24h, seven days) restores the whole machine and is the only thing
  that covers total loss. The Upstash mirror (`slimForBackup`, currently unconfigured) strips by
...
/api/admin/backups/:from/diff/:to`, where `:to` may be `today`), which is the question twelve days
  ...
  carries `certified_totals` from `captureScoreSnapshot`, so a restored file is checkable rather
  than merely present. It is armed at boot **regardless of `sd.season_closed`** — unlike the four
  schedulers that flag turns off — because the offseason is exactly when a quiet corruption goes
...
- **Archiving a season is a one-time compaction that must be INVISIBLE, and it proves that rather
  than asserting it.** `js/seasonArchive.js` (canonical, unit-tested) ↔ `server.js` does the work;
  `POST /api/seasons/:year/archive` captures per-manager totals with `captureScoreSnapshot` before
  and after and **refuses to write on any difference at all**. `force` overrides the four
  preconditions (season closed, no refused corrections, no rollup drift, not the active season) but
  NOT the totals check — a compaction that moves a point is not a compaction. The keep-set comes
  from `weekRosterWindows` over every manager × every week, never from a stat row's `manager` field.
  The tiers are CUMULATIVE and the measured ordering is counter-intuitive: tier 3 (drop derived
  caches, shrink pools) saves 0.09 MB while tier 4 (drop the `cumulative` that duplicates `delta`,
  drop zero-valued stat keys) saves 0.92 MB — ten times as much — so tier 4 is the default.
  **`sd.archived` is then a hard gate**: `rebuild-weeklies`, `recompute-scores`, `apply-corrections`,
  `/api/mlb/sync` and `/reopen` all 409, because each rebuilds from the daily rows and those are now
